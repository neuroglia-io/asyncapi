...
public class AvroSchemaHandler(IJsonSerializer jsonSerializer)
    : ISchemaHandler
{
    ...
    public virtual async Task<IOperationResult> ValidateAsync(object graph, object schema, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            var json = JsonSerializer.SerializeToText(schema);
            avroSchema = Schema.Parse(json);
        }
        catch (Exception ex)
        {
            return new OperationResult((int)HttpStatusCode.BadRequest, null, new Error()
            {
